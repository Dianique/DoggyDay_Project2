...
interface PetApiService { //Interfaces serve as custom types that aren't instantiated directly but define forms of behavior for other types to implement
    // https://dog.ceo/api/breeds/image/random
    @GET("breeds/image/random") //GET annotation points to a specific API endpoint
    suspend fun getRandomPet(): Dog //a function/method that defines getRandomDog as the Dogs data class. The suspend keyword makes this function usable in the coroutine.

}

object PetImageApi { //DogImageApi is a singleton object instantiated once within the class and usable throughout the program;
    // The object serves to initialize the retrofit service that communicates with the network.
    val retrofitService: PetApiService by lazy { retrofit.create(PetApiService::class.java) }
}
